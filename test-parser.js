...
// Get the parser from the window object
const ArbitrageLogParser = window.ArbitrageLogParser;
...
sequences.forEach((sequence, index) => {
    console.log(`\nSequence ${index + 1}:`);
    console.log(`Total Parts: ${sequence.parts.length}`);
    
    sequence.parts.forEach((part, partIndex) => {
        console.log(`\n  Part ${partIndex + 1}:`);
        console.log(`  Profit: $${part.profit.toFixed(2)}`);
        console.log(`  Commission: $${part.commission.toFixed(2)}`);
        console.log(`  Net: $${(part.profit - part.commission).toFixed(2)}`);
        
        if (part.orders && part.orders.length > 0) {
            console.log(`  Orders: ${part.orders.length}`);
            part.orders.forEach((order, orderIndex) => {
                console.log(`    Order ${orderIndex + 1}: ${order.broker || 'Unknown'} ${order.direction} ${order.lotSize} ${order.instrument} at ${order.openPrice}`);
                if (order.closePrice) {
                    console.log(`    Closed at: ${order.closePrice}`);
                }
            });
        }
        
        if (part.virtualOrders && part.virtualOrders.length > 0) {
            console.log(`  Virtual Orders: ${part.virtualOrders.length}`);
        }
    });
    
    console.log(`\n  Sequence Totals:`);
    console.log(`  Total Profit: $${sequence.totalProfit.toFixed(2)}`);
    console.log(`  Total Commission: $${sequence.totalCommission.toFixed(2)}`);
    console.log(`  Net Profit: $${sequence.netProfit.toFixed(2)}`);
});
